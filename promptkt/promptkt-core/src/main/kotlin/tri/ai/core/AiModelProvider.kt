...
/** Provides a set of AI model providers at runtime. */
interface AiModelProvider {
    ...
    /** Provide a list of text-to-speech models. */
    fun textToSpeechModels(): List<TextToSpeechModel> = emptyList()
    ...
    companion object {
        ...
        private val plugins: ServiceLoader<AiModelProvider> by lazy {
            ServiceLoader.load(AiModelProvider::class.java, pluginLoader)
        }

        val defaultPlugin by lazy { 
            plugins.firstOrNull { it.modelSource() == "OpenAI" } 
                ?: plugins.firstOrNull() 
                ?: throw IllegalStateException("No AI model providers found")
        }
        ...
        fun textToSpeechModels() = orderedPlugins.flatMap { it.textToSpeechModels() }
        ...
        /** Get a text-to-speech model by id. Throws an exception if not found. */
        fun textToSpeechModel(modelId: String) =
            textToSpeechModels().first { it.matchesModelId(modelId) }
