...
class ToolChainExecutorTest {
    ...
    @Test
    fun testPrompts() {
        tri.ai.core.agent.impl.PROMPTS.list().forEach { println(it.id) }
        assertEquals(7, tri.ai.core.agent.impl.PROMPTS.list().size)
    }
    ...
    fun testTools() {
        ...
        val tool1 = object : ToolExecutable("Calculator", "Use this to do math") {
            override suspend fun run(input: String, context: ExecContext) = ToolExecutableResult("42")
        }
        val tool2 = object : ToolExecutable("Romanizer", "Converts numbers to Roman numerals") {
            override suspend fun run(input: String, context: ExecContext) = input.toInt().let {
                when (it) {
                    42 -> ToolExecutableResult("XLII")
                    84 -> ToolExecutableResult("LXXXIV")
                    else -> ToolExecutableResult("I don't know")
                }
            }
        }

        val flow = ToolChainExecutor(listOf(tool1, tool2)).sendMessage(
    ...
    fun testTools2() {
        ...
        val tool1 = object :
            ToolExecutable("Data Query", "Use this to search for data that is needed to answer a question") {
            override suspend fun run(input: String, context: ExecContext) = OpenAiCompletionChat().complete(
                input,
                tokens = 500
            ).firstValue.let { ToolExecutableResult(it.textContent()) }
        }
        val tool2 = object : ToolExecutable(
            "Timeline",
            "Use this once you have all the data needed to show the result on a timeline. Provide structured data as input."
        ) {
            override suspend fun run(input: String, context: ExecContext) = OpenAiCompletionChat().complete(
                """
        ...
        """.trimIndent(), tokens = 1000
            ).firstValue.let { ToolExecutableResult(it.textContent(), isTerminal = true) }
        }
        val flow = ToolChainExecutor(listOf(tool1, tool2)).sendMessage(
            AgentChatSession(config = CHAT_CONFIG),
    ...
    fun testTools3() {
        ...
        fun String.complete() = runBlocking { GPT35.complete(this@complete).firstValue.textContent() }
        ...
        val exec = ToolChainExecutor(tool_list)
        val session = AgentChatSession(config = CHAT_CONFIG)
        ...
        sendMessage("Identify and extract the main concepts from the paper CoolMath.pdf, then use these concepts to generate a visual map that illustrates the structure of the study and the connections between its different parts.")
        sendMessage("Locate and extract text from the abstract and conclusion of the paper CoolMath.pdf. Analyze the sentiment of these sections to determine the tone used when discussing the research implications.")
    }

    fun tool(name: String, description: String, op: (String) -> String) = object : ToolExecutable(name, description) {
        override suspend fun run(input: String, context: ExecContext) = ToolExecutableResult(op(input))
    }
