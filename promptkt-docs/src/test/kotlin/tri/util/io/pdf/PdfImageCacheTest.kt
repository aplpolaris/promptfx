...
class PdfImageCacheTest {
    ...
    @BeforeEach
    fun setUp() {
        PdfImageCache.clearCache()
    }

    @Test
    fun testCacheInitiallyEmpty() {
        val stats = PdfImageCache.getCacheStats()
    ...
    fun testGetImagesFromNonexistentFile() {
        ...
        val images = PdfImageCache.getImagesFromPdf(nonExistentFile)
    ...
    fun testGetImagesFromNonPdfFile() {
        ...
        try {
            val images = PdfImageCache.getImagesFromPdf(textFile)
            assertEquals(0, images.size)
        } finally {
            textFile.delete()
        }
    ...
    fun testClearCache() {
        ...
        val initialStats = PdfImageCache.getCacheStats()
        ...
        val afterClearStats = PdfImageCache.getCacheStats()
